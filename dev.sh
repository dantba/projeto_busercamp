...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=brennercard.busercamp.com.br	
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development brennercard npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop brennercard
    docker rm brennercard
    docker run --name brennercard -d --env-file /home/ubuntu/brennercard.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/brennercard:/dkdata \
        brennercard start_web.sh
}

function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./brennercard/
...
echo_green "Welcome to brennercard's dev env"
