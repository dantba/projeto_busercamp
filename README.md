...
```bash
dk bash  # starts bash inside "brennercard" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and brennercard.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `brennercard` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The brennercard container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv brennercard -p python3  # creates a python3 virtualenv
...
Install docker and nginx. Create an empty postgres database brennercard owned by a user brennercard.

On your remote machine, create a file ~/brennercard.env:
...
```
server {
    server_name  brennercard.example.com;
    ...
    location /static {
        alias /home/ubuntu/dkdata/brennercard/static;
...
(Replace "brennercard.example.com" with your production domain)
